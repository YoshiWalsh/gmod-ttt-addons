...
SWEP.EquipMenuData = {
	type = "Melee Weapon",
	desc = "Looks just like a regular crowbar, but does 45 damage per hit and can push people with 3 times as much force.\nDeveloped by Joshua Walsh (YM_Industries)."
};
...
function SWEP:SecondaryAttack()
   ...
   if tr.Hit and IsValid(tr.Entity) and tr.Entity:IsPlayer() and (self.Owner:EyePos() - tr.HitPos):Length() < 100 then
      ...
      if SERVER and (not ply:IsFrozen()) then
         local pushvel = tr.Normal * GetConVar("ttt_crowbar_pushforce"):GetFloat() * 3
...
function SWEP:GetClass()
	return "weapon_ttt_bettercrowbar"
end
