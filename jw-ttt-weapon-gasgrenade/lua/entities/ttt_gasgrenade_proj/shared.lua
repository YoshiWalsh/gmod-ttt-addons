...
function ENT:Initialize()
   if not self:GetRadius() then self:SetRadius(120) end
...
function ENT:Explode(tr)
   if SERVER then
	  ...
	  local gasCloud = ents.Create("ttt_gascloud")
      gasCloud:SetPos(pos)
		--gasCloud:SetOwner(self:GetOwner())
		--gasCloud:SetThrower(self:GetThrower())
		gasCloud:Spawn()
