   ...
   for k, target in pairs(ents.FindInSphere(pos, radius)) do
      if IsValid(target) then
         ...
         if target:IsPlayer() and (not target:IsFrozen()) and ((not target.was_pushed) or target.was_pushed.t != CurTime()) then
			...
			local tr = util.QuickTrace(target:GetShootPos(), target:GetAimVector() * 32, target)
			if not tr.HitWorld then
				--[[wep = target:GetActiveWeapon()			--DROP ACTIVE WEAPON
				
				if wep.AllowDrop then
					WEPS.DropNotifiedWeapon(target, wep, false)
				end]]
				
				for k,wep in pairs(target:GetWeapons()) do			--DROP ALL WEAPONS
					if wep.AllowDrop and math.random(1,10)<=7 then
						WEPS.DropNotifiedWeapon(target, wep, false)
					end
				end
			end
